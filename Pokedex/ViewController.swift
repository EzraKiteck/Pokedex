    ...
    @IBAction func submitButtonPressed(_ sender: Any) {
        ...
        Alamofire.request(requestURL).responseJSON { (response) in
            ...
            case .success(let value):
                ...
                //Gets second pokemonType and changes bg color
                if (json["types"].arrayValue).count == 2 {
                    self.pokemonType2 = (json["types"].arrayValue)[1]["type"]["name"].stringValue
                } else {
                    self.pokemonType2 = "default"
                }
                print(self.pokemonType2)
                /*if json["types"].arrayValue)[1]["type"]["name"].stringValue != nil {
                    var pokemonType2 = json["types"].arrayValue)[1]["type"]["name"].stringValue
                } else {
                    self.descriptionTextView.backgroundColor = .white
                }*/
                if self.pokemonType2 == "" {
                    self.descriptionTextView.backgroundColor = .white
                } else {
